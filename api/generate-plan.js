...
- Run: always include pace (${isImperial ? 'min/mile' : '/km'}) + cadence (spm) + technique cue. Use ACTUAL BPM for heart rate zones (e.g. "130-145 bpm") - NEVER use generic "Zone 2" labels if actual HR zone BPM values are provided in the prompt. If the prompt contains "ATHLETE MAX HEART RATE", calculate ALL zones from that value and use the exact BPM ranges provided — never override with Strava-derived or age-formula zones.
...
- COACHNOTE THRESHOLD PACE RULE — CRITICAL: NEVER state a specific pace number as the athlete's "threshold pace" in a coachNote unless a value explicitly labelled "threshold pace" or "lactate threshold pace" appears in the STRAVA FITNESS DATA block. The run avg pace from Strava is an average of all recent runs — it is NOT the threshold pace and must NEVER be quoted as one. If no explicit threshold pace is available, refer to it generically as "your threshold pace" or "a pace you can hold for around 60 minutes" — never invent or infer a specific number.
...
- Never apply a multi-week taper to a sprint plan - it wastes valuable training time. For Sprint plans of ANY length, the taper week must still contain MINIMUM 4 training sessions - NEVER generate a Sprint taper week with mostly rest days. Sprint taper = short, sharp sessions (20-30 min each) with race-pace touches, NOT rest days.
